...
def _history_view(state, history):
    ...
    availability = state.get("schedule_availability", {})
    availability_labels = {"available": "Schedule available", "waiting_for_schedule": "Waiting for schedule", "no_games_available": "No games available yet"}
    ...
    values = [("Current schedule availability", availability_labels.get(availability.get("status"), "Schedule available")), ("Publisher note", availability.get("publisher_message")), ("Last successful check", state.get("last_successful_check") or state.get("last_schedule_document_found")), ("Last website scan", state.get("last_website_scan")), ("Last schedule document found", state.get("last_schedule_document_found")), ("Last schedule change", revisions[0].get("detected_at") if revisions else None), ("Last successful parse", state.get("last_successful_parsed")), ("Last calendar reconciliation", state.get("last_successful_calendar")), ("Last email", state.get("last_successful_email")), ("Last completed schedule update", state.get("last_successfully_completed_run")), ("Revisions in last 7 days", recent), ("Current/last source PDF", revisions[0].get("source_url") if revisions else None)]
...
def _availability_notice(availability):
    status = availability.get("status")
    if status == "waiting_for_schedule":
        note = availability.get("publisher_message")
        publisher = f'<p><b>Publisher note:</b> {_esc(note)}</p>' if note else ""
        return f'<section class="availability"><h2>Waiting for schedule</h2><p>KVA is reachable and the monitor is healthy. The current schedule document does not contain games yet.</p>{publisher}</section>'
    if status == "no_games_available":
        return '<section class="availability"><h2>No games available yet</h2><p>The current schedule document does not contain playable games yet. The monitor will continue checking automatically.</p></section>'
    return ""
...
def _page(view="home", season=None, message="", error=""):
    ...
    failure, availability = state.get("last_failure"), state.get("schedule_availability", {})
    if failure:
        health, status_class = f'Monitor problem: {failure.get("stage", "processing")}', "unhealthy"
    elif availability.get("status") in {"waiting_for_schedule", "no_games_available"}:
        health, status_class = "Waiting for schedule", "informational"
    else:
